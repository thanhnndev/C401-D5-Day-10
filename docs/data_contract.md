...
> `owner_team` đã được cập nhật trong YAML: **"Day10 Monitoring and Data Quality"**.
...
- Alert channel: `group_report_and_runbook` (đã cấu hình trong YAML).

---

## 8. Chính sách quarantine (từ YAML)

| Khi nào | Hành động |
|---------|-----------|
| `doc_id` không trong allowlist | Quarantine |
| `effective_date` không chuẩn hóa được | Quarantine |
| Record là HR version cũ | Quarantine |
| Record không đạt điều kiện publish | Quarantine |
| `chunk_id` không còn trong cleaned snapshot | Drop khỏi Chroma index |

**Quy tắc phục hồi:**
- Không chỉnh sửa artifact bằng tay
- Rerun pipeline để restore cleaned và manifest

## 9. Ownership (từ YAML)

| Vai trò | Trách nhiệm |
|---------|-------------|
| Ingestion Owner | Raw export |
| Cleaning Owner | Cleaning / quarantine |
| Monitoring Owner | Freshness / manifest / runbook |
| Embed Owner | Chroma publish boundary |
